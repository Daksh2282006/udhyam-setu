...
export const FeasibilityReportView: React.FC = () => {
  ...
  // Set browser tab title for clean print header
  useEffect(() => {
    const originalTitle = document.title;
    document.title = `UdyamSetu Feasibility Report — ${profile.name}`;
    return () => {
  ...
  const handleDownloadPdfFile = async () => {
    ...
    try {
      ...
      const filename = `UdyamSetu_Feasibility_Report_${profile.name.replace(/\s+/g, '_')}_${profile.location.district}.pdf`;
      pdf.save(filename);
    } catch (err) {
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6 animate-in fade-in duration-300 pb-16">
      ...
      <div className="no-print bg-surface-container-lowest p-4 rounded-2xl border border-outline-variant/30 card-shadow flex flex-wrap items-center justify-between gap-3 sticky top-20 z-40">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <button
            ...
            title="Open clean 2-page print dialog"
          >
          ...
          <button
            ...
            className="px-3.5 py-1.5 bg-emerald-800 text-white hover:bg-emerald-900 rounded-lg text-xs font-semibold flex items-center gap-1.5 shadow-xs cursor-pointer disabled:opacity-50"
          ...
          {/* Official Bank DPR Export */}
          <button
            onClick={handleExportBankDpr}
            className="px-3 py-1.5 bg-surface-container-high hover:bg-surface-container text-primary rounded-lg text-xs font-semibold flex items-center gap-1.5 border border-outline-variant/40 cursor-pointer"
            title="Download Bank DPR format"
          >
          ...
          <button
            ...
            className="px-3.5 py-1.5 bg-primary hover:bg-primary/90 text-white rounded-lg text-xs font-semibold flex items-center gap-1 shadow-xs cursor-pointer"
          >
            <span>Dashboard</span>
      ...
      {/* ========================================================================= */}
      {/* CAPTURE WRAPPER FOR PDF EXPORT & PRINTING                                */}
      {/* ========================================================================= */}
      <div ref={reportRef} id="feasibility-report-root" className="space-y-6 print:space-y-0">

        {/* ========================================================================= */}
        {/* PAGE 1: FEASIBILITY REPORT (EXACT REPRODUCTION OF USER REFERENCE)         */}
        {/* ========================================================================= */}
        <div className="report-page bg-white p-8 sm:p-10 rounded-2xl border border-slate-300 shadow-md print:shadow-none print:border-none print:p-8 print:m-0 space-y-6 text-slate-800">
          
          {/* Document Header */}
          <div className="border-b-2 border-emerald-800/80 pb-4">
            <div className="text-[11px] text-slate-500 font-mono flex items-center justify-between">
              <span>{formattedDateTime}</span>
              <span className="font-semibold">UdyamSetu Feasibility Report — {profile.name}</span>
            </div>
            ...
            <div className="text-xs text-slate-600 mt-2 flex flex-wrap items-center gap-2">
              <span className="flex items-center gap-1">
                💼 Business Category: <span className="font-medium text-slate-800">{isDairy ? '🐄 Dairy' : '🌾 ' + profile.category.titleEn}</span>
              </span>
              <span>•</span>
              <span className="flex items-center gap-1">
              ...
              <span>•</span>
              <span>Generated by UdyamSetu AI Platform</span>
            </div>
          </div>
          ...
          <div className="border border-slate-200 rounded-xl p-5 space-y-3 bg-white">
            ...
            <div className="divide-y divide-slate-100 text-xs">
              ...
              <div className="py-2 flex items-center justify-between">
                ...
                <span className="font-medium text-slate-900">{isDairy ? '🐄 Dairy' : '🌾 ' + profile.category.titleEn}</span>
              </div>
              ...
              <div className="py-2 flex items-center justify-between">
                ...
                <span className="font-medium text-slate-900">₹{profile.finance.ownCapital >= 100000 ? `${Math.round(profile.finance.ownCapital / 1000)}K` : profile.finance.ownCapital.toLocaleString('en-IN')}</span>
              </div>
              ...
              <div className="py-2 flex items-center justify-between">
                ...
                <span className="font-medium text-slate-900">{profile.targetCustomers || (isDairy ? 'Residential Areas Household' : 'Local Kirana Stores & Mandi')}</span>
              </div>
            </div>
          </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 text-xs">
              <div className="p-3 border border-slate-200 rounded-lg bg-slate-50/50">
                ...
                <div className="text-base font-bold text-emerald-800 mt-0.5">
                  ₹{isDairy ? '10.0L' : formatK(profile.finance.totalOutlay)}
                </div>
              </div>
          ...
          <div className="border border-slate-200 rounded-xl p-5 space-y-3 bg-white">
            ...
            <ul className="space-y-2 text-xs text-slate-700">
              ...
              <li className="flex items-start gap-2">
                <span className="text-emerald-700 font-bold">•</span>
                <span>
                  <strong className="text-slate-900">🎯 Target Customers:</strong> {profile.targetCustomers || (isDairy ? 'Residential Areas Household' : 'Local Kirana Stores & Mandi')}.
                </span>
              </li>
            </ul>
          </div>
          ...
          <div className="pt-4 border-t border-slate-200 flex justify-between items-center text-[11px] text-slate-400 font-mono">
            <span>UdyamSetu Autonomous MSME Advisory • SIH 2026</span>
            <span>1/2</span>
          </div>
        </div>

        {/* ========================================================================= */}
        {/* PAGE 2: RISK, DOCUMENTS & ACTION PLAN (PAGE 2 OF USER REFERENCE)          */}
        {/* ========================================================================= */}
        <div className="report-page bg-white p-8 sm:p-10 rounded-2xl border border-slate-300 shadow-md print:shadow-none print:border-none print:p-8 print:m-0 space-y-6 text-slate-800 print:break-before-page">
          ...
          <div className="border border-slate-200 rounded-xl p-5 space-y-3 bg-white">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-xs pt-1">
              {[
                ...
                { id: 'loan_app', label: isDairy ? 'Bank Loan Application (NABARD format)' : 'Bank Loan Application (Mudra / PMEGP format)' },
                { id: 'income', label: 'Income Certificate' },
                { id: 'subsidy_form', label: isDairy ? 'Subsidy Application Form (DEDS/DIDF)' : 'PMEGP Online Subsidy Claim Form' },
              ].map(doc => {
          ...
          <div className="pt-6 border-t border-slate-200 space-y-3">
            <div className="flex flex-wrap items-center justify-center gap-4 text-[11px] text-slate-600">
              ...
              <span className="flex items-center gap-1.5">
                ...
                <span>Estimated by UdyamSetu AI</span>
              </span>
